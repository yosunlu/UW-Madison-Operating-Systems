...
cowuvm(pde_t *pgdir, uint sz)
{
  ...
  if((d = setupkvm()) == 0)
    return 0;
  for(i = 0; i < sz; i += PGSIZE){ // sz: Size of process memory (bytes)
    if((pte = walkpgdir(pgdir, (void*)i, 0)) == 0)
...
void cowHandler(void){
  uint flags;
  uint va = rcr2();  // the va that caused page fault
  pte_t *pte;
  pte = walkpgdir(proc->pgdir, (char*)va, 0);
  uint pa = PTE_ADDR(*pte);
  
  if(proc == 0){
    panic("cowHandler: proc = 0");
    return;
  }
  
  if( pte == 0 || (*pte & PTE_P) == 0 || (*pte & PTE_U) == 0 || va > USERTOP ){  
  //if((*pte & PTE_P) == 0 || *pte == 0){  
  ...
  flags = PTE_FLAGS(*pte);
  int ref_cnt = refCount(pa / PGSIZE); // reference count for the pysical page

  if(ref_cnt <= 1){
    *pte += 2; // set the writer bit
    lcr3(PADDR(proc->pgdir));
  } 
  else if (ref_cnt > 1)
  { 
    // If the page has more than one reference, copy the page and replace it with a writeable copy in the local process
    char* mem;
    if((mem = kalloc()) == 0){
      kill(proc->pid);
    }
    decPageRef(pa / PGSIZE);
    memmove(mem, (char*)pa, PGSIZE);
    *pte = (uint)mem | flags | PTE_W | PTE_P | PTE_U;
    lcr3(PADDR(proc->pgdir));
  } 
}